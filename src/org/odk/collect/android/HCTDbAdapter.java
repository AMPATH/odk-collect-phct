...
public class HCTDbAdapter {
    ...
    private static final String TABLE_CREATE_HOUSEHOLD =
    	"create table " + SharedConstants.HOUSEHOLD + " (" + KEY_ROWID + " integer primary key autoincrement, "
    			+ KEY_ID + " text not null, " + KEY_HOUSEHOLD_HEAD + " text);";
    private static final String TABLE_CREATE_HCT =
        "create table " + SharedConstants.INDIVIDUAL + " (" + KEY_ROWID + " integer primary key autoincrement, "
    ...
    private static class DatabaseHelper extends SQLiteOpenHelper {
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
            ...
            db.execSQL("DROP TABLE IF EXISTS " + SharedConstants.HOUSEHOLD);
            db.execSQL("DROP TABLE IF EXISTS " + SharedConstants.INDIVIDUAL);
            onCreate(db);
        }
    }
    ...
    public void insertID(String tableName, String id, String parent_id) {
        ...
        if (tableName.equalsIgnoreCase(SharedConstants.HOUSEHOLD)){
    ...
    public Cursor getAllIDs() {
    	Cursor mCursor = null;
    	mCursor= mDb.query(SharedConstants.HOUSEHOLD, new String[] {KEY_ROWID, KEY_ID}, null, null, null, null, null);
        if (mCursor.getCount()<1){
        	mCursor.close();
        	return null;
        }
    ...
    public Cursor getHCTIDs(String household_id) {
    	Cursor mCursor = null;
    	mCursor= mDb.query(true, SharedConstants.INDIVIDUAL, new String[] {KEY_ROWID, KEY_ID}, KEY_HOUSEHOLD_ID + " like'" + household_id + "'", null, null, null, null, null);
        if (mCursor.getCount()<1){
        	mCursor.close();
        	return null;
        }
