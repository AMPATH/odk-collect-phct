...
class UploaderTask extends AsyncTask<String, Integer, Boolean> {
    ...
    private final String uploadProcessor = "ProcessFileUpload.jsp";
    UploaderListener mStateListener;
    String uploadServer,mAuth;
    
    public void setUploadServer(String newServer) {
        uploadServer = newServer;
    }
    
    public void setLoginDetails(String auth) {
        mAuth = auth;
    }
    ...
    protected Boolean doInBackground(String... values) {
        for (int i = 0; i < values.length; i++) {
            ...
            HttpPost mypost = new HttpPost(uploadServer + "/" + uploadProcessor);
            byte[] bytes = mAuth.getBytes();
    		mypost.setHeader("Authorization", "Basic " + new String(Base64.encodeBase64(bytes)));
            ...
            for(int j = 0; j < files.length; j++) {
            	this.publishProgress(i+1, files.length);
                File f = files[j];
            ...
            if (response != null && response.getStatusLine().getStatusCode() == 200) {
                ...
                //Rename submitted files
                for(int j = 0; j < files.length; j++) {
                    File f = files[j];
                    if (f.getName().endsWith(".xml") || f.getName().endsWith(".png") ||f.getName().endsWith(".jpg")) {
                    	f.renameTo(new File(f.getAbsolutePath() + "_submitted"));
                    }
